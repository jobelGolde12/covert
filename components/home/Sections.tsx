...
export function FeatureStrip() {
  ...
  return (
    <section className="py-20 md:py-24 border-b hairline" aria-labelledby="privacy-heading">
      <h2 id="privacy-heading" className="sr-only">Privacy</h2>
      <div className="mx-auto max-w-container px-5 md:px-10 lg:px-16 grid grid-cols-1 md:grid-cols-3 gap-10">
        {features.map((f) => (
          <div key={f.title}>
...
export function BrandStory() {
  return (
    <section id="story" className="py-20 md:py-30" aria-labelledby="story-heading">
      <div className="mx-auto max-w-container px-5 md:px-10 lg:px-16 grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-6">
          <h2 id="story-heading" className="text-h2 md:text-[44px] text-foreground max-w-[420px]">
            Designed to make conversions effortless.
          </h2>
        </div>
        <div className="lg:col-span-6 lg:pl-12">
          <p className="text-body text-muted max-w-[460px]">
            Thoughtful tooling, reliable engines, and a calm interface that gets out of the way.
            We built Folio so the documents you handle every day keep their layout, their fonts,
            and their privacy.
          </p>
          <div className="mt-10 grid grid-cols-3 gap-6 max-w-[460px]">
            {[
              ...
              <Reveal key={s.l} delay={i * 80}>
                ...
                <p className="text-[12px] text-muted mt-1">{s.l}</p>
              </Reveal>
            ))}
          </div>
          <Button href="#formats" variant="link" iconRight="arrow-right" className="mt-10">
            Browse all formats
          </Button>
        </div>
      </div>
...
export function QuoteCTA() {
  return (
    <section className="bg-surface py-24 md:py-32 text-center" aria-labelledby="cta-heading">
      <div className="mx-auto max-w-container px-5">
        <p className="mb-8 text-[10px] font-medium uppercase tracking-[0.16em] text-light">
          Free to start
        </p>
        <h2 id="cta-heading" className="text-h2 md:text-[44px] text-foreground text-balance">
          Ready to convert?
        </h2>
        <p className="mt-5 text-body text-muted max-w-[420px] mx-auto">
...
export function FAQ() {
  const items = [
    ...
    },
    {
      ...
      a: "Most converters upload every file to their servers. Folio keeps the most-used PDF tools entirely on your device — they work offline, are instant, and there is nothing to delete later.",
    },
    {
  ...
  return (
    <section id="faq" className="py-20 md:py-24" aria-labelledby="faq-heading">
      <div className="mx-auto max-w-container px-5 md:px-10 lg:px-16 max-w-[760px]">
        <h2 id="faq-heading" className="text-h2 mb-10">
          Questions, answered.
        </h2>
        <div className="divide-y divide-border border-y hairline">
          {items.map((item) => (
            <details key={item.q} className="group py-5">
              ...
              <p className="mt-3 text-body-sm text-muted max-w-[620px]">{item.a}</p>
            </details>
          ))}
        </div>
      </div>
